 ...
 * @since 2.16.0
 */
public class OllamaToolTest extends MockAITest {

    OllamaChatModel chatModelWithAutoToolCall;
    OllamaChatModel chatModelWithManualToolCall;
    ...
    public void testToolCallWithAutoToolCall() {
        setupChatMock(ProviderEnum.OLLAMA, TestDataReader.RequestType.BLOCKING_TOOL_CALL,
        ...
        Assertions.assertEquals("北京今天的天气是晴天，温度是25°C。", response.getOutput().getContent());
    }
    ...
    public void testToolCallStreamingWithAutoToolCall() {
        ...
        Assertions.assertEquals(FinishReason.STOP, response.getFinishReason());
        Assertions.assertEquals(MessageType.ASSISTANT, response.getOutput().getMessageType());
        Assertions.assertTrue(response.getOutput().getContent().contains("QQ and 微信"));
    }
    ...
    public void testToolCallWithManualToolCall() {
        ...
        setupChatMock(ProviderEnum.OLLAMA, TestDataReader.RequestType.BLOCKING_TOOL_CALL_2);
    ...
    public void setup() {
        // 构建带有自动工具调用能力的模型实例
        chatModelWithAutoToolCall = OllamaChatModel.builder()
                // 仅填写必要参数，保证校验通过
                .model("mock-model")
                .apiUrl("mock-api-url")
        ...
        // 构建带有手动工具调用能力的模型实例
        chatModelWithManualToolCall = OllamaChatModel.builder()
