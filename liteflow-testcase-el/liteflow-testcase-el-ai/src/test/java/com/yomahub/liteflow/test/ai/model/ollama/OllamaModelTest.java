...
import com.yomahub.liteflow.test.ai.engine.tool.domain.TestTools;
...
public class OllamaModelTest {
    ...
    @Test
    public void testBlockingWithToolCall() {
        ChatConfig config = OllamaChatConfig
                .builder()
                .apiUrl("http://localhost:11434/")
                .endPoint("/api/chat")
        ...
        List<Message> messages = new ArrayList<>();
        messages.add(new UserMessage("What's the weather in Beijing?"));
        ...
        ChatRequest request = OllamaChatRequest.builder()
                ...
                .onToolsCalling(((toolCalls, context) -> {
                    LOG.info("ToolCalls: \n{}", toolCalls);
                    return toolCalls;
                }))
                .build();
    ...
    @Test
    public void testBlockingWithAutoToolCall() {
        // 开启自动工具调用
        ChatConfig config = OllamaChatConfig
                .builder()
                .apiUrl("http://localhost:11434/")
    ...
    @Test
    public void testStreamingWithAutoToolCall() {
        ChatConfig config = OllamaChatConfig
                .builder()
                .apiUrl("http://localhost:11434/")
                .endPoint("/api/chat")
